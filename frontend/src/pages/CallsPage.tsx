...
type B = { id: number; name: string; floors: number; blocked_floors: { floor: number }[] };
...
export default function CallsPage() {
  ...
  const firstOpenFloor = (b?: B): number => {
    if (!b) return 1;
    const blockedSet = new Set(b.blocked_floors.map(f => f.floor));
    for (let f = 1; f <= b.floors; f++) if (!blockedSet.has(f)) return f;
    return 1;
  };

  useEffect(() => {
    api<B[]>("/buildings").then(b => {
      setBuildings(b);
      if (b[0]) {
        setBid(b[0].id);
        setFloor(f => (f >= 1 && f <= b[0].floors && !b[0].blocked_floors.some(x => x.floor === f) ? f : firstOpenFloor(b[0])));
      }
    });
    reload();
  }, []);
  ...
  const blocked = useMemo(
    () => new Set((building?.blocked_floors ?? []).map(f => f.floor)),
    [building]
  );
  ...
  function pickBuilding(id: number) {
    ...
    setBid(id);
    const blockedSet = new Set((target?.blocked_floors ?? []).map(f => f.floor));
    if (blockedSet.has(floor) || (target && floor > target.floors)) {
      setFloor(firstOpenFloor(target));
    }
  }
  ...
  return (<>
    ...
    <div className="toolbar">
      ...
      <select value={floor} onChange={e => setFloor(Number(e.target.value))} style={{ width: 96 }}>
        {levels.map(f => (
          <option key={f} value={f} disabled={blocked.has(f)}>
            {f}F{blocked.has(f) ? "（禁停）" : ""}
          </option>
        ))}
      </select>
      ...
      <button onClick={create} disabled={blocked.has(floor)}>登记呼梯</button>
    </div>
